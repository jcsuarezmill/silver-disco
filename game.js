...
(function() {
    ...
    const MESSAGES = {
        timeOut: [
            "The rocket couldn't wait forever, Harold. You're now part of the city's permanent collection.",
            "Too slow, Harold. The city's clock has officially run out on you.",
            "You hesitated for a second too long, Harold. The rocket didn't.",
            "Your escape window has closed, Harold. Their feeding window has just opened.",
            "That one detour cost you everything, Harold. The rocket left without you.",
            "The countdown reached zero, Harold, and so did your chances.",
            "You spent too much time looking back, Harold. The future left without you.",
            "You missed the launch, Harold. The monsters send their regards.",
            "Time's up, Harold. The escape route is now a buffet line, and you're first.",
            "You'll never be late again, Harold. You'll also never be... not a zombie.",
            "The rocket is gone, Harold. And so is your hope."
        ],
        monsterDeath: [
            "Your escape plan ended abruptly, Harold. And bloodily.",
            "You zigged when you should have zagged, Harold. The horde was waiting.",
            "Your will was strong, Harold, but their hunger was stronger.",
            "They got you, Harold. Overwhelmed, outsmarted, and overeaten.",
            "Your screams became their dinner bell, Harold.",
            "A grisly end. You fought well, Harold, but they were relentless.",
            "You're one of them now, Harold. Shambling and groaning with the rest.",
            "Turns out their claws were sharper than your wits, Harold.",
            "Your great escape just turned into their great feast, Harold.",
            "That's a wrap for you, Harold. A monster-food wrap.",
            "You've been decommissioned, Harold. Violently.",
            "You gambled and lost, Harold. The house always wins. And bites."
        ],
        win: [
            "Victory, Harold! You can watch the city of monsters shrink into a bad memory.",
            "Blast off! You're trading nightmares for starry skies, Harold.",
            "A perfect escape. The city is in your rearview mirror, and you are free, Harold.",
            "Success! You can finally catch your breath, Harold.",
            "You did it, Harold. The Great Escape was a success.",
            "One small step for you, Harold, one giant leap from a city of hungry jaws.",
            "You're a legend, Harold! The one that got away.",
            "Enjoy it, Harold. Freedom smells like rocket fuel and sweet, sweet relief.",
            "The city is a shrinking dot, Harold, and your future is a blank page.",
            "Launch successful! You left the apocalypse in your dust, Harold.",
            "You beat the odds and lived to tell the tale. Good work, Harold.",
            "Safe at last, Harold. The screams of the city can't reach you up here."
        ]
    };
    ...
    const audioManager = {
        ...
        isBgmPlaying: false,
        init() {
            this.sounds.shoot = new Audio('sounds/shoot.wav');
            this.sounds.zombieHit = new Audio('sounds/zombie_hit.wav');
            this.sounds.carPass = new Audio('sounds/chaingun_fire.wav');
            this.sounds.rocketLaunch = new Audio('sounds/rocket_launch.wav');
            this.sounds.coinGet = new Audio('sounds/coin_get.wav');
            this.sounds.doorOpen = new Audio('sounds/ammo_get.wav');
            this.sounds.spit = new Audio('sounds/grenade_launch.wav');
            this.sounds.shotgunFire = new Audio('sounds/shotgun_fire.wav');
            this.sounds.ammoGet = new Audio('sounds/ammo_get.wav');
            this.sounds.laserFire = new Audio('sounds/laser_fire.wav');
            this.sounds.grenadeLaunch = new Audio('sounds/grenade_launch.wav');
            this.sounds.explosion = new Audio('sounds/explosion.wav');
            this.sounds.chaingunFire = new Audio('sounds/chaingun_fire.wav');
            this.sounds.railgunCharge = new Audio('sounds/railgun_charge.wav');
            this.sounds.railgunFire = new Audio('sounds/railgun_fire.wav');
            this.sounds.bulletHit = new Audio('sounds/zombie_hit.wav');
            this.sounds.radioActivate = new Audio('sounds/railgun_charge.wav');
            this.sounds.helicopterFire = new Audio('sounds/chaingun_fire.wav');
            // --- UPDATED: Using the new helicopter sound ---
            this.sounds.helicopterLoop = new Audio('sounds/helicopter.wav');
            this.sounds.bgm = new Audio('sounds/bgm.mp3');

            this.sounds.shotgunFire.volume = 0.7;
            this.sounds.explosion.volume = 0.8;
            this.sounds.rocketLaunch.volume = 0.9;
            this.sounds.railgunFire.volume = 0.8;
            this.sounds.shoot.volume = 0.4;
            this.sounds.laserFire.volume = 0.4;
            this.sounds.grenadeLaunch.volume = 0.6;
            this.sounds.chaingunFire.volume = 0.2;
            this.sounds.coinGet.volume = 0.5;
            this.sounds.ammoGet.volume = 0.6;
            this.sounds.doorOpen.volume = 0.7;
            this.sounds.railgunCharge.volume = 0.5;
            this.sounds.radioActivate.volume = 0.8;
            this.sounds.zombieHit.volume = 0.5;
            this.sounds.bulletHit.volume = 0.5;
            this.sounds.spit.volume = 0.4;
            this.sounds.carPass.volume = 0.1;
            this.sounds.helicopterLoop.volume = 0.4; // Adjusted volume for new sound
            this.sounds.bgm.volume = 0.2;
            this.sounds.bgm.loop = true;
            this.sounds.helicopterLoop.loop = true; // Make sure the new sound loops
        },
        ...
        hit(damage) {
            ...
            if (this.health <= 0) {
                ...
                if (Math.random() < GAME_CONSTANTS.AMMO_DROP_CHANCE) {
                    ...
                    if (dropRand < 0.22) dropType = 'shotgun';
                    else if (dropRand < 0.44) dropType = 'laser';
                    else if (dropRand < 0.66) dropType = 'chaingun';
                    else if (dropRand < 0.88) dropType = 'grenade';
                    else dropType = 'radio';
    ...
    class FogParticle {
        ...
        update() {
            ...
            this.y = this.baseY + Math.sin(this.animationTimer) * 15;
            
            // If fog drifts too far left off-screen, wrap it back to the right
            if (this.x + this.radius < cameraX - GAME_CONSTANTS.WIDTH * 0.2) {
                this.x += GAME_CONSTANTS.LEVEL_WIDTH * 0.7;
            }
        }

        draw(parallaxX, overallOpacity) {
            ctx.globalAlpha = this.opacity * overallOpacity;
            ctx.fillStyle = '#808993'; // Moody grey-blue fog color
            ctx.beginPath();
            ctx.arc(parallaxX, this.y, this.radius, 0, Math.PI * 2);
            ctx.fill();
            ctx.globalAlpha = 1.0;
        }
    }
    ...
    class Explosion { 
        constructor(x, y, maxRadius = 80) {
            this.x = x; this.y = y; this.radius = 0; this.maxRadius = maxRadius; this.life = 20; this.active = true;
            audioManager.play('explosion');
            triggerCameraShake(4, 15);
            enemies.forEach(e => { if(!e.active) return; const dx = e.x + e.width/2 - this.x; const dy = e.y + e.height/2 - this.y; if(Math.sqrt(dx*dx + dy*dy) < this.maxRadius) e.hit(10); }); 
        } 
        update() { 
            this.life--; if(this.life <= 0) this.active = false; this.radius = this.maxRadius * (1 - this.life / 20); 
        } 
        draw() { 
            ctx.globalAlpha = this.life / 20; ctx.fillStyle = '#ffcc00'; ctx.beginPath(); ctx.arc(this.x, this.y, this.radius, 0, Math.PI*2); ctx.fill(); ctx.fillStyle = '#ff4d4d'; ctx.beginPath(); ctx.arc(this.x, this.y, this.radius * 0.5, 0, Math.PI*2); ctx.fill(); ctx.globalAlpha = 1; 
        } 
    }
    ...
    function generateScenery() {
        ...
        // --- NEW: Generate fog particles for the night area ---
        fogParticles = [];
        for (let i = 0; i < 50; i++) {
    ...
    function updatePlaying() {
        ...
        if (keys.shoot && canShoot && distToRocket > 100) {
            let cooldown = 0;
            if (harold.grenadeAmmo > 0) {
                fireGrenade(); cooldown = GAME_CONSTANTS.GRENADE_COOLDOWN;
            } else if (harold.railgunAmmo > 0) { /* Handled on key up/release */
            } else if (harold.laserAmmo > 0) {
                harold.laserAmmo--; audioManager.play('laserFire');
                const laserY = harold.y + harold.height / 2 + 4; const startX = harold.x + (harold.direction === 'right' ? harold.width : 0); const endX = harold.direction === 'right' ? startX + GAME_CONSTANTS.WIDTH : startX - GAME_CONSTANTS.WIDTH;
                laserBeams.push({ x1: startX, y1: laserY, x2: endX, y2: laserY, life: 10 });
                enemies.forEach(e => { if (e.active && e.y < laserY && e.y + e.height > laserY) { const isInPath = harold.direction === 'right' ? e.x > harold.x : e.x < harold.x; if (isInPath) { e.hit(4); } } });
                cooldown = GAME_CONSTANTS.LASER_COOLDOWN;
            } else if (harold.shotgunAmmo > 0) {
                harold.shotgunAmmo--; audioManager.play('shotgunFire');
                const projY = harold.y + harold.height / 2;
                for (let i=0; i < 4; i++) { let projX = harold.x + (harold.direction === 'right' ? harold.width : -10); projectiles.push(new Projectile(projX, projY, {vx: (harold.direction === 'right' ? 1 : -1) * 12, vy:(Math.random() - 0.5) * 4, color: '#ff9d00', width: 5, height: 5, damage: 2})); }
                cooldown = GAME_CONSTANTS.SHOTGUN_COOLDOWN;
            } else if (harold.chaingunAmmo > 0) {
                harold.chaingunAmmo--; audioManager.play('chaingunFire');
                const projY = harold.y + harold.height / 2; let projX = harold.x + (harold.direction === 'right' ? harold.width : -10);
                projectiles.push(new Projectile(projX, projY, {vx: (harold.direction === 'right' ? 1 : -1) * 15 + (Math.random()-0.5)*3, vy: (Math.random()-0.5)*3, color: '#cccccc', width: 6, height:3, damage: 1}));
                cooldown = GAME_CONSTANTS.CHAINGUN_COOLDOWN;
            } else {
                let projX = harold.x + (harold.direction === 'right' ? harold.width : -10); let projY = harold.y + harold.height / 2; projectiles.push(new Projectile(projX, projY, {vx: (harold.direction === 'right' ? 1 : -1) * 12, damage: 1})); audioManager.play('shoot');
                cooldown = GAME_CONSTANTS.SHOOT_COOLDOWN;
            }
            if (cooldown > 0) { canShoot = false; setTimeout(() => canShoot = true, cooldown); }
        } else if(keys.shoot && canShoot && distToRocket < 100) {
            if (haroldCoins >= GAME_CONSTANTS.ROCKET_COST) { endGame(true); } else { uiMessage = { text: `Need ${GAME_CONSTANTS.ROCKET_COST - haroldCoins} more H-Coins!`, timer: 120 }; }
        ...
        vehicles.forEach(v => v.update()); clouds.forEach(c => c.update()); coins.forEach(c => c.update()); spitProjectiles.forEach(s => s.update()); ammoDrops.forEach(d => d.update()); grenades.forEach(g => g.update()); explosions.forEach(e => e.update()); laserBeams.forEach(b => b.life--);
        bombs.forEach(b => b.update()); rocketProjectiles.forEach(r => r.update());
        // --- NEW: Update fog particles ---
        fogParticles.forEach(f => f.update());

        projectiles = projectiles.filter(p => p.active); enemies = enemies.filter(e => e.active); coins = coins.filter(c => c.active); spitProjectiles = spitProjectiles.filter(s => s.active); ammoDrops = ammoDrops.filter(d => d.active); grenades = grenades.filter(g => g.active); explosions = explosions.filter(e => e.active); laserBeams = laserBeams.filter(b => b.life > 0);
        bombs = bombs.filter(b => b.active); rocketProjectiles = rocketProjectiles.filter(r => r.active);
    ...
    function fireGrenade() {
        harold.grenadeAmmo--; audioManager.play('grenadeLaunch');
    ...
    function updateHelicopter() {
        helicopter.lifespan--;
        helicopter.animationTimer++;
        if (helicopter.lifespan <= 0) {
            helicopter.active = false;
            audioManager.stop('helicopterLoop');
            return;
        }
        if (isSoundEnabled && audioManager.sounds.helicopterLoop && audioManager.sounds.helicopterLoop.paused) {
            audioManager.play('helicopterLoop');
        }
        if (Math.abs(helicopter.x - helicopter.targetX) < 20) {
            helicopter.targetX = (cameraX + helicopter.width / 2) + Math.random() * (GAME_CONSTANTS.WIDTH - helicopter.width);
        }
        if (helicopter.x < helicopter.targetX) {
        ...
        if (helicopter.shootTimer <= 0 && nearestEnemy) {
            const angle = Math.atan2((nearestEnemy.y + nearestEnemy.height/2) - (helicopter.y + 35), (nearestEnemy.x + nearestEnemy.width/2) - helicopter.x);
            const speed = 15;
            const vx = Math.cos(angle) * speed;
            const vy = Math.sin(angle) * speed;
            projectiles.push(new Projectile(helicopter.x, helicopter.y + 35, { vx, vy, color: '#f97316', width: 8, height: 4, damage: 2 }));
            audioManager.play('helicopterFire');
            helicopter.shootTimer = GAME_CONSTANTS.HELICOPTER_FIRE_RATE;
        }
        if (helicopter.bombTimer <= 0) {
            bombs.push(new Bomb(helicopter.x, helicopter.y + helicopter.height / 2));
            helicopter.bombTimer = GAME_CONSTANTS.HELICOPTER_BOMB_COOLDOWN;
        }
        if (helicopter.rocketTimer <= 0 && nearestEnemy) {
            const angle = Math.atan2((nearestEnemy.y + nearestEnemy.height/2) - (helicopter.y + 35), (nearestEnemy.x + nearestEnemy.width/2) - helicopter.x);
            const speed = 10;
            const vx = Math.cos(angle) * speed;
            const vy = Math.sin(angle) * speed;
            rocketProjectiles.push(new RocketProjectile(helicopter.x, helicopter.y + 35, { vx, vy, color: '#cccccc', width: 12, height: 5, damage: 8, piercing: true }));
            audioManager.play('shotgunFire');
            helicopter.rocketTimer = GAME_CONSTANTS.HELICOPTER_ROCKET_COOLDOWN;
        }
    }
    ...
    function draw() {
        ...
        if (levelProgress > 0.5) {
            const overallFogOpacity = Math.min(1, (levelProgress - 0.5) / 0.4); // Fade in from 50% to 90%
            fogParticles.forEach(fog => {
                const parallaxX = fog.x - cameraX * 0.85; // Slower parallax for depth
                fog.draw(parallaxX, overallFogOpacity);
            });
        }
        ...
        ammoDrops.forEach(d => d.draw());
        spitProjectiles.forEach(s => s.draw()); 
        projectiles.forEach(p => p.draw()); 
        grenades.forEach(g => g.draw()); 
        bombs.forEach(b => b.draw());
        rocketProjectiles.forEach(r => r.draw());
    ...
    function drawHelicopter() {
        const x = helicopter.x; const y = helicopter.y; const frame = helicopter.animationTimer;
        ctx.save(); ctx.translate(x, y);
        if (helicopter.direction === 'right') { ctx.scale(-1, 1); }
        const bodyColor = '#2d3748'; const detailColor = '#4a5568'; const darkColor = '#1a202c'; const cockpitColor = 'rgba(28, 100, 141, 0.7)';
        ctx.fillStyle = detailColor; ctx.fillRect(-100, 10, 50, 15);
        ctx.fillStyle = bodyColor; ctx.beginPath(); ctx.moveTo(-90, 10); ctx.lineTo(-105, -5); ctx.lineTo(-105, 25); ctx.closePath(); ctx.fill();
        ctx.fillStyle = bodyColor; ctx.beginPath(); ctx.moveTo(60, 25); ctx.lineTo(50, 0); ctx.lineTo(-55, 0); ctx.lineTo(-60, 25); ctx.closePath(); ctx.fill();
        ctx.fillStyle = cockpitColor; ctx.beginPath(); ctx.arc(40, 12, 22, Math.PI * 0.5, Math.PI * 1.5, false); ctx.fill();
        ctx.fillStyle = detailColor; ctx.fillRect(-10, -15, 20, 15);
        ctx.fillStyle = darkColor; ctx.fillRect(-40, 25, 60, 10); ctx.fillStyle = '#555'; for(let i=0; i<4; i++) { ctx.fillRect(-35 + i*15, 28, 8, 12); }
        ctx.fillStyle = darkColor; ctx.fillRect(30, 40, 5, 10); ctx.fillRect(-30, 40, 5, 10); ctx.fillRect(-40, 50, 80, 5);
        ctx.fillStyle = darkColor; ctx.fillRect(-5, -23, 10, 8); const rotorLength = 80; ctx.globalAlpha = 0.7;
        if (frame % 2 === 0) { ctx.fillRect(-rotorLength, -20, rotorLength * 2, 4); } else { ctx.fillRect(-4, -20 - rotorLength, 8, rotorLength * 2); }
        ctx.globalAlpha = 1; ctx.fillStyle = darkColor; ctx.globalAlpha = 0.6;
        if (frame % 4 < 2) { ctx.fillRect(-103, 2, 4, 30); } else { ctx.fillRect(-110, 10, 20, 4); }
    ...
    function endGame(isWin, failureType = null) {
        ...
        if (isWin) {
            audioManager.stopAll(); audioManager.play('doorOpen');
            allUI.forEach(el => el.style.display = 'none'); gameState = 'DOOR_OPEN'; winSequenceTimer = 0;
            gameWinMessage.innerHTML = getRandomMessage(MESSAGES.win);
        } else {
            gameState = 'GAME_OVER'; audioManager.stopAll(); touchControlsContainer.classList.remove('active');
            let message = "Harold's escape has failed.";
            if (failureType === 'timeOut') { message = getRandomMessage(MESSAGES.timeOut); } else if (failureType === 'monsterDeath') { message = getRandomMessage(MESSAGES.monsterDeath); }
            gameOverMessage.innerHTML = message; gameOverOverlay.style.display = 'flex'; gameOverOverlay.focus(); document.querySelector('h1').style.display = 'block';
        }
    }
    ...
    function setupTouchControls() {
        ...
        const addButtonListeners = (button, keyName) => {
            ['touchstart', 'mousedown'].forEach(evtName => { button.addEventListener(evtName, e => { e.preventDefault(); keys[keyName] = true; }, { passive: false }); });
            ['touchend', 'mouseup', 'mouseleave', 'touchcancel'].forEach(evtName => { button.addEventListener(evtName, e => { e.preventDefault(); keys[keyName] = false; }, { passive: false }); });
        };
        addButtonListeners(leftButton, 'left'); addButtonListeners(rightButton, 'right'); addButtonListeners(jumpButton, 'up');
        ['touchstart', 'mousedown'].forEach(evtName => { shootButton.addEventListener(evtName, handleShootPress, { passive: false }); });
        ['touchend', 'mouseup', 'mouseleave', 'touchcancel'].forEach(evtName => { shootButton.addEventListener(evtName, handleShootRelease, { passive: false }); });
    }
    ...
    function startGame() { document.querySelector('h1').style.display = 'none'; startScreenOverlay.style.display = 'none'; allUI.forEach(el => el.style.display = 'flex'); if ('ontouchstart' in window || navigator.maxTouchPoints > 0 || window.innerWidth < 769) { touchControlsContainer.classList.add('active'); } gameState = 'PLAYING'; audioManager.startBgm(); gameTimer = GAME_CONSTANTS.GAME_TIMER_DURATION; if(timerInterval) clearInterval(timerInterval); timerInterval = setInterval(() => { if (gameState === 'PLAYING') gameTimer--; }, 1000); }
    ...
    function initializeGame() {
        ...
        allUI.forEach(el => el.style.display = 'none'); touchControlsContainer.classList.remove('active');
